...
__global__ void initCamera(float* C,const float* P,int seed){if(blockIdx.x!=0||threadIdx.x!=0)return;for(int i=0;i<64;i++)C[i]=0.0f;C[12]=-0.45f;C[13]=0.63f;C[14]=0.90f;C[20]=1.18f;C[22]=1.0f;C[23]=1.0f;C[31]=(float)seed;C[44]=1.0f;fpsPose(C,make_float3(2.0f,1.72f,18.0f),PI,0.0f);}
