// CUDA WebShader 0.1.0. Generated from kernel resolvePixels.
@group(0) @binding(0) var<storage, read> b_Filtered: array<f32>;
@group(0) @binding(1) var<storage, read_write> b_History: array<f32>;
@group(0) @binding(2) var<storage, read_write> b_Pixels: array<u32>;
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  let cw_argument_index_7 = (cw_buffer_offset_0 + v_i);
  let cw_argument_index_8 = (cw_buffer_offset_0 + (v_i + 1i));
  let cw_argument_index_9 = (cw_buffer_offset_0 + (v_i + 2i));
  return vec3<f32>(b_Filtered[cw_argument_index_7], b_Filtered[cw_argument_index_8], b_Filtered[cw_argument_index_9]);
}
fn f_cw_buffer_helper_1(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  let cw_argument_index_10 = (cw_buffer_offset_0 + v_i);
  let cw_argument_index_11 = (cw_buffer_offset_0 + (v_i + 1i));
  let cw_argument_index_12 = (cw_buffer_offset_0 + (v_i + 2i));
  return vec3<f32>(b_History[cw_argument_index_10], b_History[cw_argument_index_11], b_History[cw_argument_index_12]);
}
fn f_cw_buffer_helper_2(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_arg_v: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  var v_v: vec3<f32> = cw_arg_v;
  b_History[(cw_buffer_offset_0 + v_i)] = v_v.x;
  b_History[(cw_buffer_offset_0 + (v_i + 1i))] = v_v.y;
  b_History[(cw_buffer_offset_0 + (v_i + 2i))] = v_v.z;
}
...
fn main(
  ...
  var v_i: i32 = ((v_y * cw_params.p_width) + v_x);
  var v_b: i32 = (v_i * 4i);
  var v_current: vec3<f32> = f_cw_buffer_helper_0(0i, v_b, cw_thread, cw_block, cw_grid);
  var cw_tmp_5: f32;
  if ((b_C[10i] > 0.5f)) {
    cw_tmp_5 = 0.0f;
  } else {
  ...
  var v_count: f32 = cw_tmp_5;
  var cw_tmp_6: vec3<f32>;
  if ((v_count < 0.5f)) {
    cw_tmp_6 = v_current;
  } else {
    cw_tmp_6 = f_mix3(f_cw_buffer_helper_1(0i, v_b, cw_thread, cw_block, cw_grid), v_current, cw_divide_f32(1.0f, (v_count + 1.0f)), cw_thread, cw_block, cw_grid);
  }
  var v_mean: vec3<f32> = cw_tmp_6;
  f_cw_buffer_helper_2(0i, v_b, v_mean, cw_thread, cw_block, cw_grid);
  b_History[(v_b + 3i)] = (v_count + 1.0f);
  var v_col: vec3<f32> = (v_mean * vec3<f32>(b_C[20i]));
  v_col = vec3<f32>(f_aces(v_col.x, cw_thread, cw_block, cw_grid), f_aces(v_col.y, cw_thread, cw_block, cw_grid), f_aces(v_col.z, cw_thread, cw_block, cw_grid));
  v_col = vec3<f32>(cw_pow_f32(v_col.x, cw_divide_f32(1.0f, 2.2f)), cw_pow_f32(v_col.y, cw_divide_f32(1.0f, 2.2f)), cw_pow_f32(v_col.z, cw_divide_f32(1.0f, 2.2f)));
  var v_u: f32 = ((cw_divide_f32(f32(v_x), f32(cw_params.p_width)) - 0.5f) * 2.0f);
  var v_v: f32 = ((cw_divide_f32(f32(v_y), f32(cw_params.p_height)) - 0.5f) * 2.0f);
  v_col = (v_col * vec3<f32>((1.0f - (0.09f * ((v_u * v_u) + (v_v * v_v))))));
  b_Pixels[v_i] = f_packRGB(v_col, cw_thread, cw_block, cw_grid);
}
