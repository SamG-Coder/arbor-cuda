// CUDA WebShader 0.1.0. Generated from kernel filterLighting.
@group(0) @binding(0) var<storage, read> b_Linear: array<f32>;
@group(0) @binding(1) var<storage, read> b_Guide: array<f32>;
@group(0) @binding(2) var<storage, read> b_Hit: array<f32>;
@group(0) @binding(3) var<storage, read_write> b_Filtered: array<f32>;
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  return vec3<f32>(b_Linear[cw_argument_index_5], b_Linear[cw_argument_index_6], b_Linear[cw_argument_index_7]);
}
fn f_cw_buffer_helper_1(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  return vec3<f32>(b_Guide[cw_argument_index_8], b_Guide[cw_argument_index_9], b_Guide[cw_argument_index_10]);
}
fn f_cw_buffer_helper_2(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_arg_v: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  ...
  var v_i: i32 = cw_arg_i;
  var v_v: vec3<f32> = cw_arg_v;
  b_Filtered[(cw_buffer_offset_0 + v_i)] = v_v.x;
  b_Filtered[(cw_buffer_offset_0 + (v_i + 1i))] = v_v.y;
  b_Filtered[(cw_buffer_offset_0 + (v_i + 2i))] = v_v.z;
}
...
fn main(
  ...
  var v_col: vec3<f32> = f_cw_buffer_helper_0(0i, v_b, cw_thread, cw_block, cw_grid);
  var v_base: vec3<f32> = (f_cw_buffer_helper_1(0i, v_b, cw_thread, cw_block, cw_grid) + vec3<f32>(0.02f, 0.02f, 0.02f));
  var v_sum: vec3<f32> = vec3<f32>(0.0f, 0.0f, 0.0f);
  var v_weight: f32 = 0.0f;
  var v_id: i32 = i32(b_Hit[(v_b + 1i)]);
  var v_depth: f32 = b_Hit[v_b];
  if (((v_depth < 100000.0f) && (b_C[15i] < 0.5f))) {
    {
      var v_j: i32 = (-2i);
      loop {
        if (!(v_j <= 2i)) { break; }
        {
          ...
          loop {
            if (!(v_k <= 2i)) { break; }
            var v_xx: i32 = (v_x + v_k);
            var v_yy: i32 = (v_y + v_j);
            if (((((v_xx < 0i) || (v_yy < 0i)) || (v_xx >= cw_params.p_width)) || (v_yy >= cw_params.p_height))) {
              continue;
            }
            var v_q: i32 = (((v_yy * cw_params.p_width) + v_xx) * 4i);
            if ((((i32(b_Hit[(v_q + 1i)]) != v_id) || (i32(b_Hit[(v_q + 2i)]) != i32(b_Hit[(v_b + 2i)]))) || (abs((b_Hit[v_q] - v_depth)) > max(0.005f, (b_Hit[(v_b + 3i)] * 6.0f))))) {
              continue;
            }
            var v_w: f32 = exp(((-f32(((v_j * v_j) + (v_k * v_k)))) * 0.3f));
            var v_c: vec3<f32> = f_cw_buffer_helper_0(0i, v_q, cw_thread, cw_block, cw_grid);
            var v_g: vec3<f32> = (f_cw_buffer_helper_1(0i, v_q, cw_thread, cw_block, cw_grid) + vec3<f32>(0.02f, 0.02f, 0.02f));
            v_sum = (v_sum + (vec3<f32>(cw_divide_f32(v_c.x, v_g.x), cw_divide_f32(v_c.y, v_g.y), cw_divide_f32(v_c.z, v_g.z)) * vec3<f32>(v_w)));
            v_weight = (v_weight + v_w);
            continuing {
              v_k += i32(1);
            }
          }
    ...
    if ((v_weight > 0.0f)) {
      v_col = f_mix3(v_col, ((v_sum / vec3<f32>(v_weight)) * v_base), 0.86f, cw_thread, cw_block, cw_grid);
    }
  }
  f_cw_buffer_helper_2(0i, v_b, v_col, cw_thread, cw_block, cw_grid);
  b_Filtered[(v_b + 3i)] = 1.0f;
}
