// CUDA WebShader 0.1.0. Generated from kernel updateCamera.
@group(0) @binding(0) var<storage, read_write> b_C: array<f32>;
@group(0) @binding(1) var<storage, read> b_I: array<f32>;
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  return vec3<f32>(b_C[cw_argument_index_8], b_C[cw_argument_index_9], b_C[cw_argument_index_10]);
}
fn f_cw_buffer_helper_1(cw_buffer_arg_0: i32, cw_buffer_arg_1: i32, cw_arg_preset: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var cw_buffer_offset_1: i32 = cw_buffer_arg_1;
  var v_preset: i32 = cw_arg_preset;
  if ((v_preset == 1i)) {
    f_cw_buffer_helper_3((cw_buffer_offset_0 + 0i), vec3<f32>(2.0f, 1.72f, 18.0f), 3.14159265359f, 0.0f, cw_thread, cw_block, cw_grid);
  }
  if ((v_preset == 2i)) {
    f_cw_buffer_helper_3((cw_buffer_offset_0 + 0i), vec3<f32>((-1.4f), 1.72f, 7.0f), (3.14159265359f * 0.94f), 0.02f, cw_thread, cw_block, cw_grid);
  }
  if ((v_preset == 3i)) {
    f_cw_buffer_helper_3((cw_buffer_offset_0 + 0i), vec3<f32>(8.0f, 1.72f, (-3.0f)), (3.14159265359f * 0.55f), (-0.02f), cw_thread, cw_block, cw_grid);
  }
  if ((v_preset == 4i)) {
    f_cw_buffer_helper_3((cw_buffer_offset_0 + 0i), vec3<f32>((-11.0f), 1.72f, (-7.0f)), 0.65f, 0.03f, cw_thread, cw_block, cw_grid);
  }
  if ((v_preset == 5i)) {
    f_cw_buffer_helper_3((cw_buffer_offset_0 + 0i), vec3<f32>(0.0f, 12.0f, 72.0f), 3.14159265359f, (-0.12f), cw_thread, cw_block, cw_grid);
  }
  b_C[(cw_buffer_offset_0 + 9i)] = 0.0f;
  b_C[(cw_buffer_offset_0 + 10i)] = 1.0f;
}
fn f_cw_buffer_helper_2(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_arg_v: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  ...
  b_C[(cw_buffer_offset_0 + v_i)] = v_v.x;
  b_C[(cw_buffer_offset_0 + (v_i + 1i))] = v_v.y;
  b_C[(cw_buffer_offset_0 + (v_i + 2i))] = v_v.z;
}
fn f_cw_buffer_helper_3(cw_buffer_arg_0: i32, cw_arg_eye: vec3<f32>, cw_arg_yaw: f32, cw_arg_pitch: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_eye: vec3<f32> = cw_arg_eye;
  var v_yaw: f32 = cw_arg_yaw;
  var v_pitch: f32 = cw_arg_pitch;
  b_C[(cw_buffer_offset_0 + 16i)] = v_yaw;
  b_C[(cw_buffer_offset_0 + 17i)] = v_pitch;
  b_C[(cw_buffer_offset_0 + 27i)] = v_yaw;
  b_C[(cw_buffer_offset_0 + 28i)] = v_pitch;
  f_cw_buffer_helper_2((cw_buffer_offset_0 + 0i), 0i, v_eye, cw_thread, cw_block, cw_grid);
  var v_cp: f32 = cos(v_pitch);
  var v_f: vec3<f32> = f_norm3(vec3<f32>((sin(v_yaw) * v_cp), sin(v_pitch), (cos(v_yaw) * v_cp)), cw_thread, cw_block, cw_grid);
  f_cw_buffer_helper_2((cw_buffer_offset_0 + 0i), 4i, (v_eye + v_f), cw_thread, cw_block, cw_grid);
  b_C[(cw_buffer_offset_0 + 3i)] = 1.0f;
  b_C[(cw_buffer_offset_0 + 18i)] = 1.0f;
}
...
fn main(
  ...
  if (((cw_block.x != u32(0i)) || (cw_thread.x != u32(0i)))) {
    return;
  }
  b_C[10i] = 0.0f;
  b_C[24i] = (b_C[24i] + 1.0f);
  var v_dt: f32 = f_clampf(cw_params.p_delta, 0.0f, 0.1f, cw_thread, cw_block, cw_grid);
  var v_old: vec3<f32> = f_cw_buffer_helper_0(0i, 0i, cw_thread, cw_block, cw_grid);
  if ((b_I[5i] > 0.0f)) {
    f_cw_buffer_helper_1(0i, 0i, i32(b_I[5i]), cw_thread, cw_block, cw_grid);
  }
  if ((b_I[7i] > 0.0f)) {
    var cw_tmp_3: f32;
    if ((b_C[11i] > 0.0f)) {
  ...
  if ((b_I[9i] > 0.0f)) {
    b_C[15i] = f32(((i32(b_C[15i]) + 1i) % 4i));
    b_C[10i] = 1.0f;
  }
  if ((b_I[10i] > 0.0f)) {
    b_C[22i] = (1.0f - b_C[22i]);
    b_C[10i] = 1.0f;
  }
  if ((b_I[8i] != 0.0f)) {
    b_C[12i] = (b_C[12i] + ((b_I[8i] * v_dt) * 0.6f));
    b_C[10i] = 1.0f;
  }
  if ((b_I[12i] != 0.0f)) {
    b_C[20i] = f_clampf((b_C[20i] + (b_I[12i] * v_dt)), 0.25f, 3.0f, cw_thread, cw_block, cw_grid);
    b_C[10i] = 1.0f;
  }
  if ((b_C[11i] > 0.0f)) {
    b_C[8i] = (b_C[8i] + v_dt);
  ...
  b_C[16i] = (b_C[16i] - (b_I[0i] * 0.00225f));
  b_C[17i] = f_clampf((b_C[17i] - (b_I[1i] * 0.002f)), (-1.45f), 1.45f, cw_thread, cw_block, cw_grid);
  var v_yaw: f32 = b_C[16i];
  let cw_argument_index_4 = 17i;
  var v_cp: f32 = cos(b_C[cw_argument_index_4]);
  let cw_argument_index_5 = 17i;
  var v_look: vec3<f32> = f_norm3(vec3<f32>((sin(v_yaw) * v_cp), sin(b_C[cw_argument_index_5]), (cos(v_yaw) * v_cp)), cw_thread, cw_block, cw_grid);
  var v_walkF: vec3<f32> = f_norm3(vec3<f32>(sin(v_yaw), 0.0f, cos(v_yaw)), cw_thread, cw_block, cw_grid);
  var v_right: vec3<f32> = f_norm3(vec3<f32>(v_walkF.z, 0.0f, (-v_walkF.x)), cw_thread, cw_block, cw_grid);
  var cw_tmp_6: f32;
  if ((b_I[16i] > 0.5f)) {
    cw_tmp_6 = 10.5f;
  } else {
  ...
  var v_speed: f32 = cw_tmp_6;
  var v_move: vec3<f32> = ((v_walkF * vec3<f32>(b_I[15i])) + (v_right * vec3<f32>(b_I[13i])));
  if ((f_dot3(v_move, v_move, cw_thread, cw_block, cw_grid) > 0.0001f)) {
    v_move = ((f_norm3(v_move, cw_thread, cw_block, cw_grid) * vec3<f32>(v_speed)) * vec3<f32>(v_dt));
  }
  var v_eye: vec3<f32> = (f_cw_buffer_helper_0(0i, 0i, cw_thread, cw_block, cw_grid) + v_move);
  v_eye.y = 1.72f;
  f_cw_buffer_helper_2(0i, 0i, v_eye, cw_thread, cw_block, cw_grid);
  f_cw_buffer_helper_2(0i, 4i, (v_eye + v_look), cw_thread, cw_block, cw_grid);
  b_C[27i] = b_C[16i];
  b_C[28i] = b_C[17i];
  b_C[3i] = 1.0f;
  b_C[18i] = 1.0f;
  if ((((f_length3((v_eye - v_old), cw_thread, cw_block, cw_grid) > 0.000001f) || (b_I[0i] != 0.0f)) || (b_I[1i] != 0.0f))) {
    b_C[10i] = 1.0f;
  }
  if (((b_C[32i] != f32(cw_params.p_width)) || (b_C[33i] != f32(cw_params.p_height)))) {
    b_C[10i] = 1.0f;
    b_C[32i] = f32(cw_params.p_width);
    b_C[33i] = f32(cw_params.p_height);
  }
  var cw_tmp_7: f32;
  if ((b_C[10i] > 0.5f)) {
    cw_tmp_7 = 0.0f;
  } else {
    cw_tmp_7 = min(255.0f, (b_C[9i] + 1.0f));
  }
  b_C[9i] = cw_tmp_7;
}
