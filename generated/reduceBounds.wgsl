// CUDA WebShader 0.1.0. Generated from kernel reduceBounds.
@group(0) @binding(0) var<storage, read_write> b_Nodes: array<f32>;
struct CWParams {
  p_first: i32,
  cw_pad_4: u32,
  cw_pad_8: u32,
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  let cw_argument_index_3 = (cw_buffer_offset_0 + v_i);
  let cw_argument_index_4 = (cw_buffer_offset_0 + (v_i + 1i));
  let cw_argument_index_5 = (cw_buffer_offset_0 + (v_i + 2i));
  return vec3<f32>(b_Nodes[cw_argument_index_3], b_Nodes[cw_argument_index_4], b_Nodes[cw_argument_index_5]);
}
...
fn main(
  ...
  if ((v_i >= cw_params.p_first)) {
    return;
  }
  var v_b: i32 = ((cw_params.p_first + v_i) * 8i);
  var v_a: i32 = (v_b * 2i);
  var v_c: i32 = (v_a + 8i);
  f_cw_buffer_helper_1(0i, v_b, f_min3(f_cw_buffer_helper_0(0i, v_a, cw_thread, cw_block, cw_grid), f_cw_buffer_helper_0(0i, v_c, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  f_cw_buffer_helper_1(0i, (v_b + 4i), f_max3(f_cw_buffer_helper_0(0i, (v_a + 4i), cw_thread, cw_block, cw_grid), f_cw_buffer_helper_0(0i, (v_c + 4i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  b_Nodes[(v_b + 3i)] = (b_Nodes[(v_a + 3i)] + b_Nodes[(v_c + 3i)]);
  b_Nodes[(v_b + 7i)] = (-1.0f);
}
