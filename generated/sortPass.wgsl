// CUDA WebShader 0.1.0. Generated from kernel sortPass.
@group(0) @binding(0) var<storage, read_write> b_Keys: array<u32>;
@group(0) @binding(1) var<storage, read_write> b_Order: array<u32>;
struct CWParams {
  p_distance: i32,
  p_span: i32,
  cw_pad_8: u32,
...
fn main(
  ...
  var v_j: i32 = (v_i ^ cw_params.p_distance);
  if ((v_j <= v_i)) {
    return;
  }
  var v_a: u32 = b_Keys[v_i];
  var v_b: u32 = b_Keys[v_j];
  var v_ia: u32 = b_Order[v_i];
  var v_ib: u32 = b_Order[v_j];
  var v_greater: bool = ((v_a > v_b) || ((v_a == v_b) && (v_ia > v_ib)));
  var v_ascending: bool = ((v_i & cw_params.p_span) == 0i);
  if ((v_greater == v_ascending)) {
    b_Keys[v_i] = v_b;
    b_Keys[v_j] = v_a;
    b_Order[v_i] = v_ib;
    b_Order[v_j] = v_ia;
