...
@group(0) @binding(4) var<storage, read> b_Hit: array<f32>;
@group(0) @binding(5) var<storage, read_write> b_Linear: array<f32>;
@group(0) @binding(6) var<storage, read_write> b_Guide: array<f32>;
struct CWParams {
  p_width: i32,
  p_height: i32,
  cw_pad_8: u32,
...
fn f_sky(cw_arg_rd: vec3<f32>, cw_arg_sun: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  var v_h: f32 = f_sat(v_rd.y, cw_thread, cw_block, cw_grid);
  var v_c: vec3<f32> = f_mix3(vec3<f32>(0.75f, 0.79f, 0.77f), vec3<f32>(0.19f, 0.35f, 0.54f), cw_pow_f32(v_h, 0.48f), cw_thread, cw_block, cw_grid);
  var v_warm: f32 = cw_pow_f32(f_sat(f_dot3(v_rd, v_sun, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid), 32.0f);
  v_c = (v_c + (vec3<f32>(0.48f, 0.26f, 0.075f) * vec3<f32>(v_warm)));
  var v_disc: f32 = f_smooth(0.99996f, 0.999984f, f_dot3(v_rd, v_sun, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  v_c = (v_c + (vec3<f32>(8.0f, 6.7f, 4.9f) * vec3<f32>(v_disc)));
  var v_clouds: f32 = f_noise(cw_divide_f32((v_rd.x * 5.0f), (0.2f + v_h)), cw_divide_f32((v_rd.z * 5.0f), (0.2f + v_h)), 231i, cw_thread, cw_block, cw_grid);
  var v_veil: f32 = ((f_smooth(0.62f, 0.81f, v_clouds, cw_thread, cw_block, cw_grid) * f_smooth(0.02f, 0.18f, v_h, cw_thread, cw_block, cw_grid)) * 0.38f);
  return f_mix3(v_c, vec3<f32>(1.1f, 1.1f, 1.05f), v_veil, cw_thread, cw_block, cw_grid);
}
fn f_groundColor(cw_arg_p: vec3<f32>, cw_arg_fp: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var v_p: vec3<f32> = cw_arg_p;
  var v_fp: f32 = cw_arg_fp;
  var v_a: f32 = f_noise((v_p.x * 3.0f), (v_p.z * 3.0f), 819i, cw_thread, cw_block, cw_grid);
  var v_b: f32 = f_noise((v_p.x * 15.0f), (v_p.z * 15.0f), 28i, cw_thread, cw_block, cw_grid);
  var v_r: f32 = sqrt(((v_p.x * v_p.x) + (v_p.z * v_p.z)));
  var v_soil: f32 = (1.0f - f_smooth(4.1f, 6.8f, (v_r + f_noise(v_p.x, v_p.z, 13i, cw_thread, cw_block, cw_grid)), cw_thread, cw_block, cw_grid));
  var v_grass: vec3<f32> = f_mix3(vec3<f32>(0.085f, 0.115f, 0.035f), vec3<f32>(0.23f, 0.24f, 0.092f), v_a, cw_thread, cw_block, cw_grid);
  var v_dirt: vec3<f32> = f_mix3(vec3<f32>(0.075f, 0.055f, 0.035f), vec3<f32>(0.16f, 0.13f, 0.08f), v_b, cw_thread, cw_block, cw_grid);
  var v_moss: f32 = f_smooth(0.53f, 0.73f, v_a, cw_thread, cw_block, cw_grid);
  v_dirt = f_mix3(v_dirt, vec3<f32>(0.049f, 0.079f, 0.022f), (v_moss * 0.72f), cw_thread, cw_block, cw_grid);
  var v_grit: f32 = ((f_noise((v_p.x * 200.0f), (v_p.z * 200.0f), 127i, cw_thread, cw_block, cw_grid) - 0.5f) * f_bandWeight(0.007f, v_fp, cw_thread, cw_block, cw_grid));
  return (f_mix3(v_grass, v_dirt, v_soil, cw_thread, cw_block, cw_grid) * vec3<f32>((1.0f + (v_grit * 0.4f))));
}
fn f_hemiDirection(cw_arg_n: vec3<f32>, cw_arg_r1: f32, cw_arg_r2: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var v_n: vec3<f32> = cw_arg_n;
  var v_r1: f32 = cw_arg_r1;
  var v_r2: f32 = cw_arg_r2;
  var v_u: vec3<f32> = f_basisU(v_n, cw_thread, cw_block, cw_grid);
  var v_v: vec3<f32> = f_cross3(v_n, v_u, cw_thread, cw_block, cw_grid);
  var v_a: f32 = ((2.0f * 3.14159265359f) * v_r1);
  var v_s: f32 = sqrt(v_r2);
  return f_norm3((((v_u * vec3<f32>((cos(v_a) * v_s))) + (v_v * vec3<f32>((sin(v_a) * v_s)))) + (v_n * vec3<f32>(sqrt((1.0f - v_r2))))), cw_thread, cw_block, cw_grid);
}
fn f_specGGX(cw_arg_n: vec3<f32>, cw_arg_v: vec3<f32>, cw_arg_l: vec3<f32>, cw_arg_rough: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var v_n: vec3<f32> = cw_arg_n;
  var v_v: vec3<f32> = cw_arg_v;
  var v_l: vec3<f32> = cw_arg_l;
  var v_rough: f32 = cw_arg_rough;
  var v_h: vec3<f32> = f_norm3((v_v + v_l), cw_thread, cw_block, cw_grid);
  var v_nh: f32 = f_sat(f_dot3(v_n, v_h, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_nv: f32 = max(0.002f, f_sat(f_dot3(v_n, v_v, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid));
  var v_nl: f32 = f_sat(f_dot3(v_n, v_l, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_vh: f32 = f_sat(f_dot3(v_v, v_h, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_a: f32 = (v_rough * v_rough);
  var v_aa: f32 = (v_a * v_a);
  var v_den: f32 = (((v_nh * v_nh) * (v_aa - 1.0f)) + 1.0f);
  var v_D: f32 = cw_divide_f32(v_aa, ((3.14159265359f * v_den) * v_den));
  var v_k: f32 = (((v_rough + 1.0f) * (v_rough + 1.0f)) * 0.125f);
  var v_G: f32 = cw_divide_f32((cw_divide_f32(v_nv, ((v_nv * (1.0f - v_k)) + v_k)) * v_nl), ((v_nl * (1.0f - v_k)) + v_k));
  var v_F: f32 = (0.035f + (0.965f * cw_pow_f32((1.0f - v_vh), 5.0f)));
  return cw_divide_f32(((v_D * v_G) * v_F), ((4.0f * v_nv) + 0.0001f));
}
...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  return f_cw_buffer_helper_9((cw_buffer_offset_0 + 0i), 0i, cw_thread, cw_block, cw_grid);
}
fn f_cw_buffer_helper_1(cw_buffer_arg_0: i32, cw_arg_x: i32, cw_arg_y: i32, cw_arg_width: i32, cw_arg_height: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  ...
  var v_f: vec3<f32> = f_cw_buffer_helper_10((cw_buffer_offset_0 + 0i), cw_thread, cw_block, cw_grid);
  var v_r: vec3<f32> = f_cw_buffer_helper_11((cw_buffer_offset_0 + 0i), cw_thread, cw_block, cw_grid);
...
fn f_cw_buffer_helper_2(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  let cw_argument_index_16 = (cw_buffer_offset_0 + 12i);
  let cw_argument_index_17 = (cw_buffer_offset_0 + 13i);
  let cw_argument_index_18 = (cw_buffer_offset_0 + 13i);
  let cw_argument_index_19 = (cw_buffer_offset_0 + 12i);
  let cw_argument_index_20 = (cw_buffer_offset_0 + 13i);
  return f_norm3(vec3<f32>((cos(b_C[cw_argument_index_16]) * cos(b_C[cw_argument_index_17])), sin(b_C[cw_argument_index_18]), (sin(b_C[cw_argument_index_19]) * cos(b_C[cw_argument_index_20]))), cw_thread, cw_block, cw_grid);
}
fn f_cw_buffer_helper_3(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_footprint: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  ...
  if ((v_type == 0i)) {
    var v_e: f32 = max(0.000035f, min(0.0005f, (v_footprint * 0.18f)));
    var v_dx: vec3<f32> = vec3<f32>(v_e, 0.0f, 0.0f);
    var v_dy: vec3<f32> = vec3<f32>(0.0f, v_e, 0.0f);
    var v_dz: vec3<f32> = vec3<f32>(0.0f, 0.0f, v_e);
    return f_norm3(vec3<f32>((f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_p + v_dx), v_footprint, cw_thread, cw_block, cw_grid) - f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_p - v_dx), v_footprint, cw_thread, cw_block, cw_grid)), (f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_p + v_dy), v_footprint, cw_thread, cw_block, cw_grid) - f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_p - v_dy), v_footprint, cw_thread, cw_block, cw_grid)), (f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_p + v_dz), v_footprint, cw_thread, cw_block, cw_grid) - f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_p - v_dz), v_footprint, cw_thread, cw_block, cw_grid))), cw_thread, cw_block, cw_grid);
  }
  if ((v_type == 1i)) {
    var v_u: vec3<f32> = f_cw_buffer_helper_13((cw_buffer_offset_0 + 0i), v_id, v_time, v_wind, cw_thread, cw_block, cw_grid);
    var v_v: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 8i), cw_thread, cw_block, cw_grid);
    var v_n: vec3<f32> = f_cw_buffer_helper_15((cw_buffer_offset_0 + 0i), v_id, v_time, v_wind, cw_thread, cw_block, cw_grid);
    var v_q: vec3<f32> = (v_p - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid));
    var v_x: f32 = (f_dot3(v_q, v_u, cw_thread, cw_block, cw_grid) - b_P[(cw_buffer_offset_0 + (v_b + 16i))]);
    var v_y: f32 = f_dot3(v_q, v_v, cw_thread, cw_block, cw_grid);
    if ((v_x < 0.0f)) {
      var v_centre: vec3<f32> = (f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid) + (v_u * vec3<f32>((v_x + b_P[(cw_buffer_offset_0 + (v_b + 16i))]))));
      return f_norm3((v_p - v_centre), cw_thread, cw_block, cw_grid);
    }
    var v_e: f32 = 0.0002f;
    let cw_argument_index_21 = (cw_buffer_offset_0 + (v_b + 7i));
    let cw_argument_index_22 = (cw_buffer_offset_0 + (v_b + 11i));
    let cw_argument_index_23 = (cw_buffer_offset_0 + (v_b + 17i));
    let cw_argument_index_24 = (cw_buffer_offset_0 + (v_b + 18i));
    let cw_argument_index_25 = (cw_buffer_offset_0 + (v_b + 7i));
    let cw_argument_index_26 = (cw_buffer_offset_0 + (v_b + 11i));
    let cw_argument_index_27 = (cw_buffer_offset_0 + (v_b + 17i));
    let cw_argument_index_28 = (cw_buffer_offset_0 + (v_b + 18i));
    var v_a: f32 = cw_divide_f32((f_leafHeight((v_x + v_e), v_y, b_P[cw_argument_index_21], b_P[cw_argument_index_22], b_P[cw_argument_index_23], b_P[cw_argument_index_24], v_footprint, cw_thread, cw_block, cw_grid) - f_leafHeight((v_x - v_e), v_y, b_P[cw_argument_index_25], b_P[cw_argument_index_26], b_P[cw_argument_index_27], b_P[cw_argument_index_28], v_footprint, cw_thread, cw_block, cw_grid)), (2.0f * v_e));
    let cw_argument_index_29 = (cw_buffer_offset_0 + (v_b + 7i));
    let cw_argument_index_30 = (cw_buffer_offset_0 + (v_b + 11i));
    let cw_argument_index_31 = (cw_buffer_offset_0 + (v_b + 17i));
    let cw_argument_index_32 = (cw_buffer_offset_0 + (v_b + 18i));
    let cw_argument_index_33 = (cw_buffer_offset_0 + (v_b + 7i));
    let cw_argument_index_34 = (cw_buffer_offset_0 + (v_b + 11i));
    let cw_argument_index_35 = (cw_buffer_offset_0 + (v_b + 17i));
    let cw_argument_index_36 = (cw_buffer_offset_0 + (v_b + 18i));
    var v_c: f32 = cw_divide_f32((f_leafHeight(v_x, (v_y + v_e), b_P[cw_argument_index_29], b_P[cw_argument_index_30], b_P[cw_argument_index_31], b_P[cw_argument_index_32], v_footprint, cw_thread, cw_block, cw_grid) - f_leafHeight(v_x, (v_y - v_e), b_P[cw_argument_index_33], b_P[cw_argument_index_34], b_P[cw_argument_index_35], b_P[cw_argument_index_36], v_footprint, cw_thread, cw_block, cw_grid)), (2.0f * v_e));
    return f_norm3(((v_n - (v_u * vec3<f32>(v_a))) - (v_v * vec3<f32>(v_c))), cw_thread, cw_block, cw_grid);
  }
  var v_q: vec3<f32> = (v_p - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid));
  var v_h: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid);
  return f_norm3(vec3<f32>(cw_divide_f32(v_q.x, (v_h.x * v_h.x)), cw_divide_f32(v_q.y, (v_h.y * v_h.y)), cw_divide_f32(v_q.z, (v_h.z * v_h.z))), cw_thread, cw_block, cw_grid);
}
fn f_cw_buffer_helper_4(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_fp: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_fp: f32 = cw_arg_fp;
  ...
  var v_wind: f32 = cw_arg_wind;
  if ((v_id < 0i)) {
    return f_groundColor(v_p, v_fp, cw_thread, cw_block, cw_grid);
  }
  var v_b: i32 = (v_id * 24i);
  var v_type: i32 = i32(b_P[(cw_buffer_offset_0 + (v_b + 3i))]);
  if ((v_type == 0i)) {
    return f_cw_buffer_helper_16((cw_buffer_offset_0 + 0i), v_id, v_p, v_fp, cw_thread, cw_block, cw_grid);
  }
  if ((v_type == 1i)) {
    return f_cw_buffer_helper_17((cw_buffer_offset_0 + 0i), v_id, v_p, v_fp, v_time, v_wind, cw_thread, cw_block, cw_grid);
  }
  var v_f: f32 = f_noise3((v_p * vec3<f32>(42.0f)), i32(b_P[(cw_buffer_offset_0 + (v_b + 15i))]), cw_thread, cw_block, cw_grid);
  return f_mix3(vec3<f32>(0.09f, 0.1f, 0.086f), vec3<f32>(0.31f, 0.3f, 0.24f), v_f, cw_thread, cw_block, cw_grid);
}
fn f_cw_buffer_helper_5(cw_buffer_arg_0: i32, cw_buffer_arg_1: i32, cw_buffer_arg_2: i32, cw_arg_p: vec3<f32>, cw_arg_direction: vec3<f32>, cw_arg_original: i32, cw_arg_originalTree: i32, cw_arg_time: f32, cw_arg_wind: f32, cw_arg_worldSeed: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var cw_buffer_offset_1: i32 = cw_buffer_arg_1;
  var cw_buffer_offset_2: i32 = cw_buffer_arg_2;
  var v_p: vec3<f32> = cw_arg_p;
  var v_direction: vec3<f32> = cw_arg_direction;
  var v_original: i32 = cw_arg_original;
  var v_originalTree: i32 = cw_arg_originalTree;
  var v_time: f32 = cw_arg_time;
  var v_wind: f32 = cw_arg_wind;
  var v_worldSeed: i32 = cw_arg_worldSeed;
  var v_transmission: vec3<f32> = vec3<f32>(1.0f, 1.0f, 1.0f);
  var v_ro: vec3<f32> = v_p;
  {
    var v_j: i32 = 0i;
    loop {
      if (!(v_j < 4i)) { break; }
      var v_tree: i32 = (-1i);
      var v_h: vec2<f32> = f_cw_buffer_helper_18((cw_buffer_offset_0 + 0i), (cw_buffer_offset_1 + 0i), (cw_buffer_offset_2 + 0i), v_ro, v_direction, 0.0001f, v_time, v_wind, 36.0f, v_worldSeed, &v_tree, true, cw_thread, cw_block, cw_grid);
      var v_id: i32 = i32(v_h.y);
      if ((v_id < 0i)) {
        break;
      }
      if ((b_P[(cw_buffer_offset_0 + ((v_id * 24i) + 3i))] != 1.0f)) {
        return vec3<f32>(0.0f, 0.0f, 0.0f);
      }
      v_transmission = (v_transmission * vec3<f32>(0.18f, 0.29f, 0.065f));
      v_ro = (v_ro + (v_direction * vec3<f32>((v_h.x + 0.0018f))));
      continuing {
        v_j += i32(1);
      }
    }
...
fn f_cw_buffer_helper_6(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_footprint: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_footprint: f32 = cw_arg_footprint;
  ...
  var v_b: i32 = (v_id * 24i);
  var v_q: vec3<f32> = (v_p - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid));
  var v_x: f32 = (f_dot3(v_q, f_cw_buffer_helper_13((cw_buffer_offset_0 + 0i), v_id, v_time, v_wind, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid) - b_P[(cw_buffer_offset_0 + (v_b + 16i))]);
  var v_y: f32 = f_dot3(v_q, f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 8i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_len: f32 = b_P[(cw_buffer_offset_0 + (v_b + 7i))];
  var v_mid: f32 = exp(cw_divide_f32((-abs(v_y)), max(0.00065f, (v_footprint * 0.55f))));
  var v_fy: f32 = f_frac((cw_divide_f32((v_x - (abs(v_y) * 0.7f)), v_len) * 7.0f), cw_thread, cw_block, cw_grid);
  var v_dd: f32 = cw_divide_f32((min(v_fy, (1.0f - v_fy)) * v_len), 7.0f);
  var v_lateral: f32 = exp(cw_divide_f32((-v_dd), max(0.00033f, (v_footprint * 0.5f))));
  return (f_sat(((v_mid * 0.82f) + (v_lateral * 0.48f)), cw_thread, cw_block, cw_grid) * f_bandWeight(0.003f, v_footprint, cw_thread, cw_block, cw_grid));
}
fn f_cw_buffer_helper_7(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_arg_v: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  var v_v: vec3<f32> = cw_arg_v;
  b_Linear[(cw_buffer_offset_0 + v_i)] = v_v.x;
  b_Linear[(cw_buffer_offset_0 + (v_i + 1i))] = v_v.y;
  b_Linear[(cw_buffer_offset_0 + (v_i + 2i))] = v_v.z;
}
fn f_cw_buffer_helper_8(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_arg_v: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  var v_v: vec3<f32> = cw_arg_v;
  b_Guide[(cw_buffer_offset_0 + v_i)] = v_v.x;
  b_Guide[(cw_buffer_offset_0 + (v_i + 1i))] = v_v.y;
  b_Guide[(cw_buffer_offset_0 + (v_i + 2i))] = v_v.z;
}
fn f_cw_buffer_helper_9(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  let cw_argument_index_37 = (cw_buffer_offset_0 + v_i);
  let cw_argument_index_38 = (cw_buffer_offset_0 + (v_i + 1i));
  let cw_argument_index_39 = (cw_buffer_offset_0 + (v_i + 2i));
  return vec3<f32>(b_C[cw_argument_index_37], b_C[cw_argument_index_38], b_C[cw_argument_index_39]);
}
fn f_cw_buffer_helper_10(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  return f_norm3((f_cw_buffer_helper_9((cw_buffer_offset_0 + 0i), 4i, cw_thread, cw_block, cw_grid) - f_cw_buffer_helper_9((cw_buffer_offset_0 + 0i), 0i, cw_thread, cw_block, cw_grid)), cw_thread, cw_block, cw_grid);
}
fn f_cw_buffer_helper_11(cw_buffer_arg_0: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  return f_norm3(f_cross3(f_cw_buffer_helper_10((cw_buffer_offset_0 + 0i), cw_thread, cw_block, cw_grid), vec3<f32>(0.0f, 1.0f, 0.0f), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
}
fn f_cw_buffer_helper_12(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_footprint: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_footprint: f32 = cw_arg_footprint;
  var v_b: i32 = (v_id * 24i);
  var v_a: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  var v_ab: vec3<f32> = (f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid) - v_a);
  ...
  let cw_argument_index_40 = (cw_buffer_offset_0 + (v_b + 7i));
  let cw_argument_index_41 = (cw_buffer_offset_0 + (v_b + 11i));
  var v_radius: f32 = f_lerp(b_P[cw_argument_index_40], b_P[cw_argument_index_41], v_t, cw_thread, cw_block, cw_grid);
  return ((f_length3(((v_p - v_a) - (v_ab * vec3<f32>(v_t))), cw_thread, cw_block, cw_grid) - v_radius) + f_cw_buffer_helper_19((cw_buffer_offset_0 + 0i), v_id, v_p, v_footprint, cw_thread, cw_block, cw_grid));
}
fn f_cw_buffer_helper_13(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_time: f32, cw_arg_wind: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_time: f32 = cw_arg_time;
  var v_wind: f32 = cw_arg_wind;
  var v_b: i32 = (v_id * 24i);
  var v_u: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid);
  var v_n: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 12i), cw_thread, cw_block, cw_grid);
...
fn f_cw_buffer_helper_14(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  let cw_argument_index_42 = (cw_buffer_offset_0 + v_i);
  let cw_argument_index_43 = (cw_buffer_offset_0 + (v_i + 1i));
  let cw_argument_index_44 = (cw_buffer_offset_0 + (v_i + 2i));
  return vec3<f32>(b_P[cw_argument_index_42], b_P[cw_argument_index_43], b_P[cw_argument_index_44]);
}
...
fn f_cw_buffer_helper_16(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_footprint: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_footprint: f32 = cw_arg_footprint;
  ...
  var v_uv: vec2<f32> = f_cw_buffer_helper_20((cw_buffer_offset_0 + 0i), v_id, v_p, cw_thread, cw_block, cw_grid);
  var v_coord: vec3<f32> = vec3<f32>((cos(v_uv.x) * 9.0f), (sin(v_uv.x) * 9.0f), (v_uv.y * 7.0f));
  var v_mottle: f32 = f_noise3((v_p * vec3<f32>(3.0f)), 714i, cw_thread, cw_block, cw_grid);
  var v_rough: f32 = f_noise3(v_coord, v_seed, cw_thread, cw_block, cw_grid);
  var v_d: vec3<f32> = (f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid) - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid));
  var v_t: f32 = f_sat(cw_divide_f32(f_dot3((v_p - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid)), v_d, cw_thread, cw_block, cw_grid), max(0.000001f, f_dot3(v_d, v_d, cw_thread, cw_block, cw_grid))), cw_thread, cw_block, cw_grid);
  let cw_argument_index_45 = (cw_buffer_offset_0 + (v_b + 7i));
  let cw_argument_index_46 = (cw_buffer_offset_0 + (v_b + 11i));
  var v_amp: f32 = min(0.021f, (f_lerp(b_P[cw_argument_index_45], b_P[cw_argument_index_46], v_t, cw_thread, cw_block, cw_grid) * 0.055f));
  var v_cut: f32 = cw_divide_f32(f_cw_buffer_helper_19((cw_buffer_offset_0 + 0i), v_id, v_p, v_footprint, cw_thread, cw_block, cw_grid), max(0.00001f, v_amp));
  var v_c: vec3<f32> = f_mix3(vec3<f32>(0.1f, 0.075f, 0.05f), vec3<f32>(0.25f, 0.21f, 0.16f), (0.25f + (v_rough * 0.62f)), cw_thread, cw_block, cw_grid);
  v_c = (v_c * vec3<f32>((0.75f + (0.35f * v_mottle))));
  v_c = (v_c * vec3<f32>((1.0f - (0.62f * f_smooth(0.28f, 0.85f, v_cut, cw_thread, cw_block, cw_grid)))));
  var v_moss: f32 = (f_smooth(0.52f, 0.72f, f_noise3((v_p * vec3<f32>(5.0f)), 189i, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid) * (1.0f - f_smooth(0.15f, 1.4f, v_p.y, cw_thread, cw_block, cw_grid)));
  v_c = f_mix3(v_c, vec3<f32>(0.045f, 0.064f, 0.018f), (v_moss * 0.62f), cw_thread, cw_block, cw_grid);
  var v_lichen: f32 = ((f_smooth(0.65f, 0.77f, f_noise3((v_p * vec3<f32>(23.0f)), 27i, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid) * f_smooth(0.25f, 0.65f, f_noise3((v_p * vec3<f32>(90.0f)), 712i, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid)) * f_bandWeight(0.018f, v_footprint, cw_thread, cw_block, cw_grid));
  v_c = f_mix3(v_c, vec3<f32>(0.31f, 0.33f, 0.23f), (v_lichen * 0.5f), cw_thread, cw_block, cw_grid);
  var v_grain: f32 = ((f_noise3((v_coord * vec3<f32>(15.0f)), v_seed, cw_thread, cw_block, cw_grid) - 0.5f) * f_bandWeight(0.0025f, v_footprint, cw_thread, cw_block, cw_grid));
  return (v_c * vec3<f32>((1.0f + (v_grain * 0.38f))));
}
fn f_cw_buffer_helper_17(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_fp: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_fp: f32 = cw_arg_fp;
  ...
  var v_wind: f32 = cw_arg_wind;
  var v_b: i32 = (v_id * 24i);
  var v_h: f32 = f_rnd(i32(b_P[(cw_buffer_offset_0 + (v_b + 15i))]), 887i, cw_thread, cw_block, cw_grid);
  var v_q: vec3<f32> = (v_p - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid));
  var v_x: f32 = (f_dot3(v_q, f_cw_buffer_helper_13((cw_buffer_offset_0 + 0i), v_id, v_time, v_wind, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid) - b_P[(cw_buffer_offset_0 + (v_b + 16i))]);
  if ((v_x < 0.0f)) {
    return vec3<f32>(0.17f, 0.15f, 0.048f);
  }
  var v_y: f32 = f_dot3(v_q, f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 8i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_v: f32 = f_cw_buffer_helper_6((cw_buffer_offset_0 + 0i), v_id, v_p, v_fp, v_time, v_wind, cw_thread, cw_block, cw_grid);
  var v_c: vec3<f32> = f_mix3(vec3<f32>(0.035f, 0.11f, 0.008f), vec3<f32>(0.15f, 0.255f, 0.025f), v_h, cw_thread, cw_block, cw_grid);
  v_c = f_mix3(v_c, vec3<f32>(0.22f, 0.26f, 0.055f), (v_v * 0.57f), cw_thread, cw_block, cw_grid);
  var v_speck: f32 = ((f_noise((v_x * 700.0f), (v_y * 700.0f), i32(b_P[(cw_buffer_offset_0 + (v_b + 15i))]), cw_thread, cw_block, cw_grid) - 0.5f) * f_bandWeight(0.0017f, v_fp, cw_thread, cw_block, cw_grid));
  let cw_argument_index_47 = (cw_buffer_offset_0 + (v_b + 7i));
  let cw_argument_index_48 = (cw_buffer_offset_0 + (v_b + 11i));
  var v_edge: f32 = f_smooth(0.78f, 1.0f, cw_divide_f32(abs(v_y), max(0.001f, f_leafWidth(v_x, b_P[cw_argument_index_47], b_P[cw_argument_index_48], v_fp, cw_thread, cw_block, cw_grid))), cw_thread, cw_block, cw_grid);
  v_c = (v_c * vec3<f32>((1.0f + (v_speck * 0.27f))));
  v_c = f_mix3(v_c, vec3<f32>(0.23f, 0.2f, 0.05f), (v_edge * 0.22f), cw_thread, cw_block, cw_grid);
  return v_c;
}
fn f_cw_buffer_helper_18(cw_buffer_arg_0: i32, cw_buffer_arg_1: i32, cw_buffer_arg_2: i32, cw_arg_ro: vec3<f32>, cw_arg_rd: vec3<f32>, cw_arg_cone: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_arg_best: f32, cw_arg_worldSeed: i32, v_treeOut: ptr<function, i32>, cw_arg_shadows: bool, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec2<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var cw_buffer_offset_1: i32 = cw_buffer_arg_1;
  var cw_buffer_offset_2: i32 = cw_buffer_arg_2;
  var v_ro: vec3<f32> = cw_arg_ro;
  ...
  var v_bx: f32 = ((f32((v_ix - (18i / 2i))) + cw_tmp_51) * 28.0f);
  var cw_tmp_52: f32;
  if ((v_sz > 0i)) {
    cw_tmp_52 = 0.5f;
  } else {
  ...
  var v_bz: f32 = ((f32((v_iz - (18i / 2i))) + cw_tmp_52) * 28.0f);
  ...
  var v_dz: f32 = (28.0f * abs(v_invz));
  {
    ...
    loop {
      ...
      if ((v_broad < v_best)) {
        ...
        var v_h: vec2<f32> = f_cw_buffer_helper_21((cw_buffer_offset_0 + 0i), (cw_buffer_offset_1 + 0i), (cw_buffer_offset_2 + 0i), v_lr, v_ld, cw_divide_f32(v_cone, v_sc), (v_time + v_phase), v_wind, cw_divide_f32(v_best, v_sc), (-1i), v_shadows, cw_thread, cw_block, cw_grid);
...
fn f_cw_buffer_helper_19(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_arg_footprint: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_footprint: f32 = cw_arg_footprint;
  var v_b: i32 = (v_id * 24i);
  var v_a: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  var v_d: vec3<f32> = (f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid) - v_a);
  ...
  let cw_argument_index_54 = (cw_buffer_offset_0 + (v_b + 7i));
  let cw_argument_index_55 = (cw_buffer_offset_0 + (v_b + 11i));
  var v_radius: f32 = f_lerp(b_P[cw_argument_index_54], b_P[cw_argument_index_55], v_along, cw_thread, cw_block, cw_grid);
...
fn f_cw_buffer_helper_20(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_p: vec3<f32>, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec2<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_p: vec3<f32> = cw_arg_p;
  var v_b: i32 = (v_id * 24i);
  var v_a: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  var v_d: vec3<f32> = f_norm3((f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid) - v_a), cw_thread, cw_block, cw_grid);
  var v_initial: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 8i), cw_thread, cw_block, cw_grid);
...
fn f_cw_buffer_helper_21(cw_buffer_arg_0: i32, cw_buffer_arg_1: i32, cw_buffer_arg_2: i32, cw_arg_ro: vec3<f32>, cw_arg_rd: vec3<f32>, cw_arg_cone: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_arg_best: f32, cw_arg_ignore: i32, cw_arg_shadows: bool, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec2<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var cw_buffer_offset_1: i32 = cw_buffer_arg_1;
  var cw_buffer_offset_2: i32 = cw_buffer_arg_2;
  var v_ro: vec3<f32> = cw_arg_ro;
  ...
  var v_found: i32 = (-1i);
  {
    ...
    loop {
      ...
      var v_span: vec2<f32> = f_boxSpan(v_ro, v_rd, f_cw_buffer_helper_22((cw_buffer_offset_1 + 0i), v_b, cw_thread, cw_block, cw_grid), f_cw_buffer_helper_22((cw_buffer_offset_1 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
      ...
      if ((v_hit && (v_node < 32768i))) {
        ...
        var v_ls: vec2<f32> = f_boxSpan(v_ro, v_rd, f_cw_buffer_helper_22((cw_buffer_offset_1 + 0i), v_lb, cw_thread, cw_block, cw_grid), f_cw_buffer_helper_22((cw_buffer_offset_1 + 0i), (v_lb + 4i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
        var v_rs: vec2<f32> = f_boxSpan(v_ro, v_rd, f_cw_buffer_helper_22((cw_buffer_offset_1 + 0i), v_rb, cw_thread, cw_block, cw_grid), f_cw_buffer_helper_22((cw_buffer_offset_1 + 0i), (v_rb + 4i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
        ...
        if (v_hit) {
          ...
          if ((v_id != v_ignore)) {
            ...
            var v_t: f32 = f_cw_buffer_helper_23((cw_buffer_offset_0 + 0i), v_id, v_ro, v_rd, v_footprint, v_time, v_wind, (!v_shadows), cw_thread, cw_block, cw_grid);
            if ((v_t < v_best)) {
              v_best = v_t;
              v_found = v_id;
            }
...
fn f_cw_buffer_helper_22(cw_buffer_arg_0: i32, cw_arg_i: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_i: i32 = cw_arg_i;
  let cw_argument_index_58 = (cw_buffer_offset_0 + v_i);
  let cw_argument_index_59 = (cw_buffer_offset_0 + (v_i + 1i));
  let cw_argument_index_60 = (cw_buffer_offset_0 + (v_i + 2i));
  return vec3<f32>(b_Nodes[cw_argument_index_58], b_Nodes[cw_argument_index_59], b_Nodes[cw_argument_index_60]);
}
fn f_cw_buffer_helper_23(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_ro: vec3<f32>, cw_arg_rd: vec3<f32>, cw_arg_footprint: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_arg_refine: bool, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_ro: vec3<f32> = cw_arg_ro;
  var v_rd: vec3<f32> = cw_arg_rd;
  ...
  if ((v_type == 0i)) {
    return f_cw_buffer_helper_24((cw_buffer_offset_0 + 0i), v_id, v_ro, v_rd, v_footprint, v_refine, cw_thread, cw_block, cw_grid);
  }
  if ((v_type == 1i)) {
    return f_cw_buffer_helper_25((cw_buffer_offset_0 + 0i), v_id, v_ro, v_rd, v_footprint, v_time, v_wind, cw_thread, cw_block, cw_grid);
  }
  var v_h: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid);
  var v_q: vec3<f32> = (v_ro - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid));
...
fn f_cw_buffer_helper_24(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_ro: vec3<f32>, cw_arg_rd: vec3<f32>, cw_arg_footprint: f32, cw_arg_refine: bool, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_ro: vec3<f32> = cw_arg_ro;
  var v_rd: vec3<f32> = cw_arg_rd;
  ...
  var v_b: i32 = (v_id * 24i);
  var v_a: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  var v_end: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid);
  if ((!v_refine)) {
  ...
  let cw_argument_index_62 = (cw_buffer_offset_0 + (v_b + 7i));
  let cw_argument_index_63 = (cw_buffer_offset_0 + (v_b + 11i));
  var v_t: f32 = f_tubeT(v_ro, v_rd, v_a, v_end, b_P[cw_argument_index_62], b_P[cw_argument_index_63], cw_thread, cw_block, cw_grid);
  if ((v_t >= 100000.0f)) {
    return 100000.0f;
  }
  var v_span: vec2<f32> = f_boxSpan(v_ro, v_rd, f_cw_buffer_helper_26((cw_buffer_offset_0 + 0i), v_id, cw_thread, cw_block, cw_grid), f_cw_buffer_helper_27((cw_buffer_offset_0 + 0i), v_id, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
  var v_eps: f32 = max(0.000025f, min(0.00025f, (v_footprint * 0.13f)));
  {
    var v_k: i32 = 0i;
    loop {
      if (!(v_k < 64i)) { break; }
      var v_f: f32 = f_cw_buffer_helper_12((cw_buffer_offset_0 + 0i), v_id, (v_ro + (v_rd * vec3<f32>(v_t))), v_footprint, cw_thread, cw_block, cw_grid);
      if ((v_f < v_eps)) {
        return v_t;
      }
...
fn f_cw_buffer_helper_25(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_arg_ro: vec3<f32>, cw_arg_rd: vec3<f32>, cw_arg_footprint: f32, cw_arg_time: f32, cw_arg_wind: f32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> f32 {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_ro: vec3<f32> = cw_arg_ro;
  var v_rd: vec3<f32> = cw_arg_rd;
  ...
  var v_wind: f32 = cw_arg_wind;
  var v_b: i32 = (v_id * 24i);
  var v_origin: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  var v_u: vec3<f32> = f_cw_buffer_helper_13((cw_buffer_offset_0 + 0i), v_id, v_time, v_wind, cw_thread, cw_block, cw_grid);
  var v_v: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 8i), cw_thread, cw_block, cw_grid);
  var v_n: vec3<f32> = f_cw_buffer_helper_15((cw_buffer_offset_0 + 0i), v_id, v_time, v_wind, cw_thread, cw_block, cw_grid);
  ...
  if ((abs(v_dz) > 0.000001f)) {
    cw_tmp_64 = cw_divide_f32((-v_z), v_dz);
  } else {
    cw_tmp_64 = ((v_span.x + v_span.y) * 0.5f);
  }
  var v_t: f32 = cw_tmp_64;
  v_t = f_clampf(v_t, max(0.0001f, v_span.x), v_span.y, cw_thread, cw_block, cw_grid);
  {
    ...
    loop {
      ...
      let cw_argument_index_65 = (cw_buffer_offset_0 + (v_b + 17i));
      let cw_argument_index_66 = (cw_buffer_offset_0 + (v_b + 18i));
      var v_height: f32 = f_leafHeight(v_xx, v_yy, v_len, v_width, b_P[cw_argument_index_65], b_P[cw_argument_index_66], v_footprint, cw_thread, cw_block, cw_grid);
      var v_f: f32 = ((v_z + (v_dz * v_t)) - v_height);
      var v_e: f32 = 0.00015f;
      let cw_argument_index_67 = (cw_buffer_offset_0 + (v_b + 17i));
      let cw_argument_index_68 = (cw_buffer_offset_0 + (v_b + 18i));
      let cw_argument_index_69 = (cw_buffer_offset_0 + (v_b + 17i));
      let cw_argument_index_70 = (cw_buffer_offset_0 + (v_b + 18i));
      var v_hx: f32 = cw_divide_f32((f_leafHeight((v_xx + v_e), v_yy, v_len, v_width, b_P[cw_argument_index_67], b_P[cw_argument_index_68], v_footprint, cw_thread, cw_block, cw_grid) - f_leafHeight((v_xx - v_e), v_yy, v_len, v_width, b_P[cw_argument_index_69], b_P[cw_argument_index_70], v_footprint, cw_thread, cw_block, cw_grid)), (2.0f * v_e));
      let cw_argument_index_71 = (cw_buffer_offset_0 + (v_b + 17i));
      let cw_argument_index_72 = (cw_buffer_offset_0 + (v_b + 18i));
      let cw_argument_index_73 = (cw_buffer_offset_0 + (v_b + 17i));
      let cw_argument_index_74 = (cw_buffer_offset_0 + (v_b + 18i));
      var v_hy: f32 = cw_divide_f32((f_leafHeight(v_xx, (v_yy + v_e), v_len, v_width, b_P[cw_argument_index_71], b_P[cw_argument_index_72], v_footprint, cw_thread, cw_block, cw_grid) - f_leafHeight(v_xx, (v_yy - v_e), v_len, v_width, b_P[cw_argument_index_73], b_P[cw_argument_index_74], v_footprint, cw_thread, cw_block, cw_grid)), (2.0f * v_e));
  ...
  var v_xx: f32 = (v_x + (v_dx * v_t));
  var v_yy: f32 = (v_y + (v_dy * v_t));
  let cw_argument_index_75 = (cw_buffer_offset_0 + (v_b + 17i));
  let cw_argument_index_76 = (cw_buffer_offset_0 + (v_b + 18i));
  var v_err: f32 = abs(((v_z + (v_dz * v_t)) - f_leafHeight(v_xx, v_yy, v_len, v_width, b_P[cw_argument_index_75], b_P[cw_argument_index_76], v_footprint, cw_thread, cw_block, cw_grid)));
...
fn f_cw_buffer_helper_26(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_b: i32 = (v_id * 24i);
  var v_type: i32 = i32(b_P[(cw_buffer_offset_0 + (v_b + 3i))]);
  ...
  var v_a: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  if ((v_type == 0i)) {
    let cw_argument_index_77 = (cw_buffer_offset_0 + (v_b + 7i));
    let cw_argument_index_78 = (cw_buffer_offset_0 + (v_b + 11i));
    var v_r: f32 = (max(b_P[cw_argument_index_77], b_P[cw_argument_index_78]) + 0.002f);
    return (f_min3(v_a, f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid) - vec3<f32>(v_r, v_r, v_r));
  }
  if ((v_type == 2i)) {
    return (v_a - f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid));
  }
...
fn f_cw_buffer_helper_27(cw_buffer_arg_0: i32, cw_arg_id: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> vec3<f32> {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var v_id: i32 = cw_arg_id;
  var v_b: i32 = (v_id * 24i);
  var v_type: i32 = i32(b_P[(cw_buffer_offset_0 + (v_b + 3i))]);
  ...
  var v_a: vec3<f32> = f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), v_b, cw_thread, cw_block, cw_grid);
  if ((v_type == 0i)) {
    let cw_argument_index_79 = (cw_buffer_offset_0 + (v_b + 7i));
    let cw_argument_index_80 = (cw_buffer_offset_0 + (v_b + 11i));
    var v_r: f32 = (max(b_P[cw_argument_index_79], b_P[cw_argument_index_80]) + 0.002f);
    return (f_max3(v_a, f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid) + vec3<f32>(v_r, v_r, v_r));
  }
  if ((v_type == 2i)) {
    return (v_a + f_cw_buffer_helper_14((cw_buffer_offset_0 + 0i), (v_b + 4i), cw_thread, cw_block, cw_grid));
  }
...
fn main(
  ...
  var v_pixel: i32 = ((v_y * cw_params.p_width) + v_x);
  var v_b: i32 = (v_pixel * 4i);
  var v_t: f32 = b_Hit[v_b];
  var v_id: i32 = i32(b_Hit[(v_b + 1i)]);
  var v_tree: i32 = i32(b_Hit[(v_b + 2i)]);
  ...
  var v_col: vec3<f32> = f_sky(v_rd, v_sun, cw_thread, cw_block, cw_grid);
  var v_guide: vec3<f32> = vec3<f32>(1.0f, 1.0f, 1.0f);
  var v_worldSeed: i32 = i32(b_C[31i]);
  if ((v_t < 100000.0f)) {
    var v_p: vec3<f32> = (v_ro + (v_rd * vec3<f32>(v_t)));
    let cw_argument_index_5 = (v_b + 3i);
    var v_fp: f32 = max(0.000005f, b_Hit[cw_argument_index_5]);
    var v_localP: vec3<f32> = v_p;
    var v_n: vec3<f32> = vec3<f32>(0.0f, 1.0f, 0.0f);
    var v_localFp: f32 = v_fp;
    var v_phase: f32 = 0.0f;
    if (((v_id >= 0i) && (v_tree >= 0i))) {
      var v_sc: f32 = f_forestScale(v_tree, v_worldSeed, cw_thread, cw_block, cw_grid);
      v_localP = f_forestToLocalPoint(v_p, v_tree, v_worldSeed, cw_thread, cw_block, cw_grid);
      v_localFp = cw_divide_f32(v_fp, v_sc);
      v_phase = (f32((f_forestSeed(v_tree, v_worldSeed, cw_thread, cw_block, cw_grid) & 8191i)) * 0.0017f);
      let cw_argument_index_6 = 11i;
      v_n = f_forestToWorldNormal(f_cw_buffer_helper_3(0i, v_id, v_localP, v_localFp, (b_C[8i] + v_phase), b_C[cw_argument_index_6], cw_thread, cw_block, cw_grid), v_tree, v_worldSeed, cw_thread, cw_block, cw_grid);
    }
    var v_leaf: bool = ((v_id >= 0i) && (b_P[((v_id * 24i) + 3i)] == 1.0f));
    if ((f_dot3(v_n, v_rd, cw_thread, cw_block, cw_grid) > 0.0f)) {
      v_n = (v_n * vec3<f32>((-1.0f)));
    }
    if ((v_id < 0i)) {
      var v_e: f32 = max(0.001f, v_fp);
      var v_nx: f32 = ((f_noise(((v_p.x + v_e) * 18.0f), (v_p.z * 18.0f), 133i, cw_thread, cw_block, cw_grid) - f_noise(((v_p.x - v_e) * 18.0f), (v_p.z * 18.0f), 133i, cw_thread, cw_block, cw_grid)) * 0.18f);
      var v_nz: f32 = ((f_noise((v_p.x * 18.0f), ((v_p.z + v_e) * 18.0f), 133i, cw_thread, cw_block, cw_grid) - f_noise((v_p.x * 18.0f), ((v_p.z - v_e) * 18.0f), 133i, cw_thread, cw_block, cw_grid)) * 0.18f);
      v_n = f_norm3(vec3<f32>(v_nx, 1.0f, v_nz), cw_thread, cw_block, cw_grid);
    }
    var cw_tmp_7: vec3<f32>;
    if ((v_id >= 0i)) {
      cw_tmp_7 = v_localP;
    } else {
      cw_tmp_7 = v_p;
    }
    let cw_argument_index_8 = 11i;
    var v_albedo: vec3<f32> = f_cw_buffer_helper_4(0i, v_id, cw_tmp_7, v_localFp, (b_C[8i] + v_phase), b_C[cw_argument_index_8], cw_thread, cw_block, cw_grid);
    if ((v_tree >= 0i)) {
      var v_ts: i32 = f_forestSeed(v_tree, v_worldSeed, cw_thread, cw_block, cw_grid);
      var v_hue: f32 = f_rnd(8i, v_ts, cw_thread, cw_block, cw_grid);
      var v_age: f32 = f_rnd(9i, v_ts, cw_thread, cw_block, cw_grid);
      var v_season: f32 = f_rnd(10i, v_ts, cw_thread, cw_block, cw_grid);
      var v_dark: f32 = (0.76f + (v_age * 0.38f));
      var v_tint: vec3<f32> = vec3<f32>((0.88f + (0.22f * v_hue)), (0.84f + (0.28f * (1.0f - abs((v_hue - 0.55f))))), (0.72f + (0.25f * (1.0f - v_hue))));
      if (v_leaf) {
        v_tint = f_mix3(v_tint, vec3<f32>(1.12f, 0.88f, 0.48f), (f_smooth(0.78f, 0.98f, v_season, cw_thread, cw_block, cw_grid) * 0.38f), cw_thread, cw_block, cw_grid);
      } else {
        v_tint = f_mix3(v_tint, vec3<f32>(0.78f, 0.66f, 0.53f), (v_season * 0.18f), cw_thread, cw_block, cw_grid);
      }
      v_albedo = ((v_albedo * v_tint) * vec3<f32>(v_dark));
    }
    v_guide = v_albedo;
    var v_sample: i32 = i32(b_C[9i]);
    var v_r1: f32 = f_rnd((v_pixel + (v_sample * 1823i)), 991i, cw_thread, cw_block, cw_grid);
    var v_r2: f32 = f_rnd((v_pixel + (v_sample * 971i)), 731i, cw_thread, cw_block, cw_grid);
    var v_su: vec3<f32> = f_basisU(v_sun, cw_thread, cw_block, cw_grid);
    var v_sv: vec3<f32> = f_cross3(v_sun, v_su, cw_thread, cw_block, cw_grid);
    var v_angle: f32 = (v_r1 * 6.28318f);
    var v_disk: f32 = (sqrt(v_r2) * 0.012f);
    var v_light: vec3<f32> = f_norm3(((v_sun + (v_su * vec3<f32>((cos(v_angle) * v_disk)))) + (v_sv * vec3<f32>((sin(v_angle) * v_disk)))), cw_thread, cw_block, cw_grid);
    var cw_tmp_9: f32;
    if ((v_id >= 0i)) {
      cw_tmp_9 = 0.0014f;
    } else {
    ...
    var v_offset: f32 = cw_tmp_9;
    var v_visibility: vec3<f32> = vec3<f32>(1.0f, 1.0f, 1.0f);
    if ((b_C[22i] > 0.5f)) {
      let cw_argument_index_10 = 8i;
      let cw_argument_index_11 = 11i;
      v_visibility = f_cw_buffer_helper_5(0i, 0i, 0i, (v_p + (v_light * vec3<f32>(v_offset))), v_light, v_id, v_tree, b_C[cw_argument_index_10], b_C[cw_argument_index_11], v_worldSeed, cw_thread, cw_block, cw_grid);
    }
    var v_nl: f32 = f_sat(f_dot3(v_n, v_light, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid);
    var v_back: f32 = f_sat((-f_dot3(v_n, v_light, cw_thread, cw_block, cw_grid)), cw_thread, cw_block, cw_grid);
    var v_direct: vec3<f32> = (v_albedo * vec3<f32>(cw_divide_f32(v_nl, 3.14159265359f)));
    if (v_leaf) {
      let cw_argument_index_12 = 11i;
      var v_vein: f32 = f_cw_buffer_helper_6(0i, v_id, v_localP, v_localFp, (b_C[8i] + v_phase), b_C[cw_argument_index_12], cw_thread, cw_block, cw_grid);
      v_direct = (v_direct + (v_albedo * vec3<f32>(((v_back * (0.75f - (0.25f * v_vein))) + (cw_pow_f32(f_sat(f_dot3(v_rd, v_light, cw_thread, cw_block, cw_grid), cw_thread, cw_block, cw_grid), 9.0f) * 0.12f)))));
    }
    var cw_tmp_13: f32;
    if (v_leaf) {
      cw_tmp_13 = 0.39f;
    ...
    var v_spec: f32 = f_specGGX(v_n, (v_rd * vec3<f32>((-1.0f))), v_light, cw_tmp_13, cw_thread, cw_block, cw_grid);
    v_direct = (v_direct + vec3<f32>(v_spec, v_spec, v_spec));
    var v_ao: f32 = 1.0f;
    var v_ambient: vec3<f32> = (((vec3<f32>(0.3f, 0.37f, 0.45f) * vec3<f32>((0.45f + (0.55f * f_sat(v_n.y, cw_thread, cw_block, cw_grid))))) * vec3<f32>(v_ao)) + (vec3<f32>(0.08f, 0.068f, 0.035f) * vec3<f32>(f_sat((-v_n.y), cw_thread, cw_block, cw_grid))));
    if (v_leaf) {
      v_ambient = (v_ambient + (vec3<f32>(0.04f, 0.055f, 0.022f) * vec3<f32>(v_ao)));
    }
    v_col = (((v_direct * v_visibility) * vec3<f32>(4.8f, 4.2f, 3.1f)) + (v_albedo * v_ambient));
    var v_haze: f32 = (1.0f - exp(((-v_t) * 0.0018f)));
    v_col = f_mix3(v_col, f_sky(v_rd, v_sun, cw_thread, cw_block, cw_grid), v_haze, cw_thread, cw_block, cw_grid);
    if ((b_C[15i] == 1.0f)) {
      var v_w0: f32 = f_bandWeight(0.065f, v_localFp, cw_thread, cw_block, cw_grid);
      var v_w1: f32 = f_bandWeight(0.005f, v_localFp, cw_thread, cw_block, cw_grid);
      var v_w2: f32 = f_bandWeight(0.001f, v_localFp, cw_thread, cw_block, cw_grid);
      v_col = vec3<f32>((v_w0 * 0.8f), (v_w1 * 0.85f), (v_w2 * 0.9f));
    }
    if ((b_C[15i] == 2.0f)) {
      var v_h: f32 = f_rnd(((v_id + 177i) + ((v_tree + 1i) * 71i)), 32i, cw_thread, cw_block, cw_grid);
      var cw_tmp_15: vec3<f32>;
      if ((v_id < 0i)) {
        cw_tmp_15 = vec3<f32>(0.1f, 0.1f, 0.1f);
      } else {
        var cw_tmp_14: vec3<f32>;
        if (v_leaf) {
          cw_tmp_14 = vec3<f32>(0.1f, (0.3f + (v_h * 0.5f)), 0.17f);
        } else {
          cw_tmp_14 = vec3<f32>((0.47f + (v_h * 0.3f)), 0.25f, 0.1f);
        }
        cw_tmp_15 = cw_tmp_14;
      }
      v_col = cw_tmp_15;
    ...
    if ((b_C[15i] == 3.0f)) {
      v_col = ((v_n * vec3<f32>(0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
    }
  }
  f_cw_buffer_helper_7(0i, v_b, v_col, cw_thread, cw_block, cw_grid);
  b_Linear[(v_b + 3i)] = 1.0f;
  f_cw_buffer_helper_8(0i, v_b, v_guide, cw_thread, cw_block, cw_grid);
  b_Guide[(v_b + 3i)] = 1.0f;
}
